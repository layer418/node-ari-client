...
var swagger = require('./swagger-client.js');
...
Client.prototype._attachApi = function () {
  ...
  return new Promise(function(resolve, reject) {
    ...
    http.get(ariUrl, {user: self._connection.user, pass: self._connection.pass})
      .then(function(response) {
        if (response.statusCode !== 200) {
          var err = new Error('Failed to connect to ARI');
      ...
      .catch(function(err) {
        if (['ETIMEDOUT', 'ENOTFOUND', 'ECONNREFUSED'].includes(err.code)) {
          err.name = 'HostIsNotReachable';
        }
        self.emit('APILoadError', err);
        reject(err);
    ...
    function swaggerLoaded () {
      if(self._swagger.ready === true) {
        ...
        Object.keys(self._swagger.apis).forEach(attachResource);

        // Attach resource creators to client
        _resources.knownTypes.forEach(attachResourceCreators);
    ...
    function attachResource (resource) {
      ...
      Object.keys(operations).forEach(attachOperation);
      ...
      function attachOperation (operation) {
        ...
        function callSwagger (/* args..., callback */) {
          ...
          var options = args[0];
          var userCallback = (typeof args[args.length - 1] === 'function') ? args[args.length - 1]
                                                          : undefined;
          ...
          var promise = new Promise(function(innerResolve, innerReject) {
            ...
            function processResponse (response) {
              ...
              if (_resources.knownTypes.includes(respType) &&
          ...
          return promiseUtils.asCallback(promise, userCallback);
        }
      }
    }
  });
...
Client.prototype.start = function (apps, subscribeAll, callback) {
  ...
  var promise = new Promise(function(resolve, reject) {
    ...
    resolve = promiseUtils.once(resolve);

    reject = promiseUtils.once(reject);

    var applications = (Array.isArray(apps)) ? apps.join(',') : apps;
    ...
    function processMessage (msg, flags) {
      ...
      var eventModel = null;
      var eventModelKeys = Object.keys(eventModels);
      for (var i = 0; i < eventModelKeys.length; i++) {
        if (eventModelKeys[i] === event.type) {
          eventModel = eventModels[eventModelKeys[i]];
          break;
        }
      }
      var resources = {};
      ...
      if (eventModel && eventModel.properties) {
        Object.keys(eventModel.properties).forEach(function (propName) {
          var prop = eventModel.properties[propName];
          if (_resources.knownTypes.includes(prop.dataType) &&
      ...
      var promoted = Object.keys(resources).length;
      if (promoted === 1) {
  ...
  return promiseUtils.asCallback(promise, callback);
};
...
module.exports.connect = function (baseUrl, user, pass,
  ...
  return promiseUtils.asCallback(client._attachApi(), callback);
};
